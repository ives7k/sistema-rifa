    ...
    <div className="fixed inset-0 bg-black/80 z-50 flex justify-center items-start pt-10 overflow-y-auto">
      <div className="bg-gray-50 rounded-lg shadow-xl w-full max-w-md mx-4 mb-10" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-4 bg-white">
          <div className="bg-gray-100 p-3 rounded-md text-sm text-gray-600 mb-4">
            <b className="text-gray-800">{quantity}</b> unidade(s) do produto <b className="text-gray-800">EDIÇÃO 76 - NOVO TERA 2026 0KM</b>
          </div>
        </div>
        ...
        {pixData ? (
          <div className="bg-gray-50 p-4 space-y-4">
            ...
            <button className="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-3 rounded-lg flex items-center justify-center space-x-2">
                <i className="bi bi-check-circle-fill"></i>
                <span>Já fiz o pagamento</span>
            </button>
            
            <div className="bg-blue-100 border-l-4 border-blue-400 text-blue-800 p-3 text-sm rounded-r-md text-center">
            ...
            <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 space-y-2">
              <h4 className="font-bold text-gray-800 border-b pb-2 mb-2">Detalhes da sua compra</h4>
              <p className="text-xs text-gray-500 break-words"><b>ID:</b> {pixData.token}</p>
              <p className="text-sm text-gray-700"><b>Comprador:</b> {pixData.comprador.nome}</p>
              <p className="text-sm text-gray-700"><b>CPF:</b> {formatCPF(pixData.comprador.cpf)}</p>
              <p className="text-sm text-gray-700"><b>Telefone:</b> {formatPhone(pixData.comprador.telefone)}</p>
              <p className="text-sm text-gray-700"><b>Situação:</b> <span className="font-semibold text-yellow-600">Aguardando pagamento</span></p>
              <p className="text-sm text-gray-700"><b>Quantidade:</b> {quantity}</p>
              <p className="text-sm text-gray-700"><b>Total:</b> {pixData.valor.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</p>
            </div>
          ...
          <div className="p-4">
            <form className="space-y-3" onSubmit={handlePayment}>
                <div>
                    <label htmlFor="phone" className="block text-sm font-bold text-gray-800 mb-1">Informe seu telefone</label>
                    <input type="tel" id="phone" name="phone" value={phone} onChange={(e) => setPhone(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent placeholder-gray-400" placeholder="(00) 00000-0000" disabled={isLoading} />
                </div>

                {error && <div className="bg-red-100 border-l-4 border-red-400 text-red-800 p-3 text-sm rounded-r-md"><i className="bi bi-x-circle-fill mr-2"></i>{error}</div>}
                
                {!phone && <div className="bg-yellow-100 border-l-4 border-yellow-400 text-yellow-800 p-3 text-sm rounded-r-md"><i className="bi bi-exclamation-circle-fill mr-2"></i>Informe seu telefone para continuar.</div>}

                <button type="submit" className="w-full bg-[#1db954] hover:bg-[#1aa34a] text-white font-bold py-3 px-4 rounded-lg flex justify-center items-center space-x-2 transition-colors disabled:bg-gray-400" disabled={isLoading || !phone}>
                    {isLoading ? (
